// リアルタイム音声翻訳 - JavaScript (GPT-5-nano版)
...
8. 簡潔で高品質な翻訳を行う。`;
    ...
    function setupSpeechRecognition() {
        ...
        // 音声認識結果の処理 - リアルタイム翻訳強化版（デバウンス最適化）
        recognition.onresult = function(event) {
            ...
            // 新しいコンテンツがある場合、翻訳をトリガー（言語別最適デバウンス適用）
            if (hasNewContent && displayText !== lastTranslatedText) {
                // 翻訳処理をデバウンス（言語に応じて動的変更）
                ...
                }, dynamicDebounce); // 言語別最適化されたデバウンス時間
            }
        };
    ...
    // OpenAI API（GPT-5-nanoモデル）を使用してテキストを翻訳
    async function translateText(text) {
        ...
        try {
            ...
            // GPT-5-nanoモデルを使用したOpenAIリクエストを作成
